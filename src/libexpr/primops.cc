...
#pragma GCC diagnostic pop
    ...
    static RegisterPrimOp primop_abort(
      ...
      Abort Nix expression evaluation and print the error message *s*.
         ...
         .fun = [](EvalState & state, const PosIdx pos, Value ** args, Value & v) {
             ...
             auto s = state
                          .coerceToString(
                              pos, *args[0], context, "while evaluating the error message passed to builtins.abort")
                          .toOwned();
    ...
    static void prim_derivationStrict(EvalState & state, const PosIdx pos, Value ** args, Value & v)
    {
        ...
        auto nameAttr =
            state.getAttr(state.sName, attrs, "in the attrset passed as argument to builtins.derivationStrict");
    ...
    static void derivationStrictInternal(EvalState & state, std::string_view drvName, const Bindings * attrs, Value & v)
    {
        ...
        if (isDerivation(drvName)
            && !(
                ingestionMethod == ContentAddressMethod::Raw::Text && outputs.size() == 1
                && *(outputs.begin()) == "out")) {
            state
                .error<EvalError>(
        ...
        if (outputHash) {
            ...
            case DrvHash::Kind::Regular:
                for (auto & i : outputs) {
                    ...
                    if (!h)
                        state.error<AssertionError>("derivation produced no hash for output '%s'", i)
                            .atPos(v)
                            .debugThrow();
    ...
    {
        auto s =
            state.forceStringNoCtx(*args[0], pos, "while evaluating the first argument passed to builtins.fromJSON");
        try {
            parseJSON(state, s, v);
    ...
    static void prim_attrNames(EvalState & state, const PosIdx pos, Value ** args, Value & v)
    {
        ...
        std::sort(
            list.begin(), list.end(), [](Value * v1, Value * v2) { return strcmp(v1->c_str(), v2->c_str()) < 0; });
    ...
    {
        auto attr =
            state.forceStringNoCtx(*args[0], pos, "while evaluating the first argument passed to builtins.getAttr");
        state.forceAttrs(*args[1], pos, "while evaluating the second argument passed to builtins.getAttr");
        auto i =
            state.getAttr(state.symbols.create(attr), args[1]->attrs(), "in the attribute set under consideration");
        // !!! add to stack trace?
    ...
    {
        auto attr =
            state.forceStringNoCtx(*args[0], pos, "while evaluating the first argument passed to builtins.hasAttr");
    ...
    static void prim_elemAt(EvalState & state, const PosIdx pos, Value ** args, Value & v)
    {
        ...
        if (n < 0 || std::make_unsigned_t<NixInt::Inner>(n) >= args[0]->listSize())
            state
                .error<EvalError>("'builtins.elemAt' called with index %d on a list of size %d", n, args[0]->listSize())
                .atPos(pos)
                .debugThrow();
    ...
    static void prim_elem(EvalState & state, const PosIdx pos, Value ** args, Value & v)
    {
        ...
        for (auto elem : args[1]->listItems())
            if (state.eqValues(
                    *args[0], *elem, pos, "while searching for the presence of the given element in the list")) {
                res = true;
                break;
            }
        v.mkBool(res);
    ...
    {
        auto len_ =
            state.forceInt(*args[1], pos, "while evaluating the second argument passed to builtins.genList").value;
            ...
            if (args[0]->isPrimOp()) {
                ...
                if (ptr && *ptr == prim_lessThan)
                    return CompareValues(
                        state, noPos, "while evaluating the ordering function passed to builtins.sort")(a, b);
            }
    ...
    static void prim_convertHash(EvalState & state, const PosIdx pos, Value ** args, Value & v)
    {
        ...
        auto iteratorHash =
            state.getAttr(state.symbols.create("hash"), inputAttrs, "while locating the attribute 'hash'");
