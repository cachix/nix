    ...
    nix_c_context * context, nix_flake_lock_flags * flags, const char * inputPath, size_t inputPathLen)
{
    ...
    try {
        ...
        if (auto nonEmpty = nix::flake::NonEmptyInputAttrPath::make(path))
            flags->lockFlags->inputUpdates.insert(std::move(*nonEmpty));
        return NIX_OK;
    }
    NIXC_CATCH_ERRS
}
    ...
    void * user_data)
{
    ...
    try {
        ...
        if (auto lockedNode = std::get_if<nix::ref<nix::flake::LockedNode>>(&edge)) {
            if (auto fingerprint = (*lockedNode)->lockedRef.input.getFingerprint(*fetchSettings->settings, *store->ptr)) {
