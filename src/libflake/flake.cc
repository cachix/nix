    ...
    bool trustLock)
{
    ...
    for (auto & [id, input2] : flakeInputs) {
        ...
        try {
            ...
            if (oldNode && !ctx.lockFlags.inputUpdates.count(*NonEmptyInputAttrPath::make(inputAttrPath)))
            ...
            if (oldLock && oldLock->originalRef.canonicalize() == input.ref->canonicalize()
                ...
                auto lb = ctx.lockFlags.inputUpdates.lower_bound(*NonEmptyInputAttrPath::make(inputAttrPath));

                auto mustRefetch = lb != ctx.lockFlags.inputUpdates.end() && lb->get().size() > inputAttrPath.size()
                    ...
                    auto [path, lockedRef] = [&]() -> std::tuple<SourcePath, FlakeRef> {
                        ...
                        if (auto resolvedPath = resolveRelativePath()) {
                            ...
                            auto cachedInput = ctx.state.inputCache->getAccessor(
                                ctx.state.fetchSettings, *ctx.state.store, input.ref->input, ctx.useRegistriesInputs);
